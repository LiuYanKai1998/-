 ...
 *
 */
public class TwoDimDocWorkflowHelper implements RemoteAccess{
	
	private static final String CLASSNAME = TwoDimDocWorkflowHelper.class.getName();
	private static final Logger logger    = Logger.getLogger(CLASSNAME);
	
	
	
	/**
	 * 二维图样,编制/修改环节选择"提交审阅"路由完成校验
	 * 1、文档不能为检出状态;
	...
	public static void submitReviewValidate(ObjectReference self,WTObject primaryBusinessObject) throws WTException{
		String errorMsg = "";
		try{ 
			logger.debug("二维图样流程的提交审阅环节校验开始:" + primaryBusinessObject.getDisplayIdentifier());
			WTDocument doc = (WTDocument) primaryBusinessObject;
			//1.检出状态校验
			if(WorkableUtil.isCheckedOut(doc)){
				errorMsg +=("当前二维图样处于检出状态，请检入后再提交编制任务\n");
			}
			...
			logger.error("二维图样文档流程的提交审阅环节校验出错",e);
		}
		if(!"".equals(errorMsg)){
	...
	public static void cancelReviewValidate(ObjectReference self,WTObject primaryBusinessObject) throws WTException{
		if(WorkableUtil.isCheckedOut((Workable)primaryBusinessObject)){
			throw new WTException("当前二维图样处于检出状态，请检入后再提交编制任务\n");
		}
	}
